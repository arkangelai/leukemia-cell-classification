...
def postprocess_boxes(pred_bbox, original_image, input_size, score_threshold):
    ...
    #print("shape pred_bbox: ", pred_bbox.shape)
...
def detect_image_batched(Yolo, image_paths, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', return_boxes=False, from_path=True, images=[]):
    ...
    if YOLO_FRAMEWORK == "tf":
        pred_bboxes = Yolo.predict(images_data)
        ...
        for key, value in result.items():
            value = value.numpy()
            pred_bboxes.append(value)
    
    bbox_list = []
    #print('pred_bboxes shape: ', len(pred_bboxes[0]))     
    for pred_bbox, original_image in zip(pred_bboxes[1], images):
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', return_boxes=False, from_path=True, image=[]):
